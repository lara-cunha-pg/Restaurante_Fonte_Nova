...
class PgProjectBudgetSyncRun(models.Model):
    _name = 'pg.project.budget.sync.run'
    _description = 'PG Project Budget Sync Run'
    ...
    snapshot_path = fields.Char(string='Snapshot Path', required=True, default='.pg/PG_BUDGET_SYNC.json')
    ...
    def _get_project_budget_sync_service(self):
        return ProjectBudgetSyncService(self.env)

    def action_process_now(self):
        for run in self:
            self._get_project_budget_sync_service().process_run(run)
        return {'type': 'ir.actions.client', 'tag': 'reload'}

    @api.model
    def _cron_process_project_budget_sync_queue(self, limit=5):
        runs = self.search([('status', '=', 'queued')], order='create_date asc, id asc', limit=limit)
        for run in runs:
            try:
                self._get_project_budget_sync_service().process_run(run)
            except Exception:
                _logger.exception("Project budget sync queue failed for run %s", run.id)
        return len(runs)
