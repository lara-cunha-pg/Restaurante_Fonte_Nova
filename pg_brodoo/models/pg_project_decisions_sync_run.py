...
class PgProjectDecisionsSyncRun(models.Model):
    _name = 'pg.project.decisions.sync.run'
    _description = 'PG Project Decisions Sync Run'
    ...
    snapshot_path = fields.Char(string='Snapshot Path', required=True, default='.pg/PG_DECISIONS_SYNC.json')
    ...
    def _get_decisions_sync_service(self):
        return ProjectDecisionsSyncService(self.env)

    def action_process_now(self):
        for run in self:
            self._get_decisions_sync_service().process_run(run)
        return {'type': 'ir.actions.client', 'tag': 'reload'}

    @api.model
    def _cron_process_decisions_sync_queue(self, limit=5):
        runs = self.search([('status', '=', 'queued')], order='create_date asc, id asc', limit=limit)
        for run in runs:
            try:
                self._get_decisions_sync_service().process_run(run)
            except Exception:
                _logger.exception("Decisions sync queue failed for run %s", run.id)
        return len(runs)
