...
from .project_status_payload_builder import ProjectStatusPayloadBuilder
...
class ProjectStatusSyncService:
    def __init__(self, env):
        self.env = env
        self.payload_builder = ProjectStatusPayloadBuilder(env)
    ...
    def _update_project_status(self, project, values):
        project.with_context(pg_skip_status_sync_touch=True).sudo().write(values)
    ...
    def queue_project(self, project, trigger_type='manual_button', trigger_model='project.project', trigger_record_id=None):
        ...
        if not project or not project.pg_status_sync_enabled:
            return False
        if not project.pg_repository_id or not (project.pg_repo_branch or '').strip():
            return False

        existing_run = self.env['pg.project.status.sync.run'].sudo().search(
        ...
        run = self.env['pg.project.status.sync.run'].sudo().create(
            {
                ...
                'snapshot_path': '.pg/PG_PROJECT_STATUS_SYNC.json',
                'message': _('Queued for status publication.'),
            }
        )
        self._update_project_status(
            project,
            {
                'pg_status_sync_last_status': 'queued',
                'pg_status_sync_last_message': _('Status sync queued.'),
            },
        )
        return run
    ...
    def process_run(self, run):
        ...
        if not project.pg_repository_id or not (project.pg_repo_branch or '').strip():
            raise UserError(_("The project must define both a repository and a repository branch for status sync."))

        run.write({'status': 'running', 'started_at': fields.Datetime.now(), 'message': _('Building payload.')})
        self._update_project_status(
            project,
            {
                'pg_status_sync_last_status': 'running',
                'pg_status_sync_last_message': _('Building project status payload.'),
            },
        )
        ...
        try:
            ...
            review = self.quality_review_service.review_status_payload(payload)
            ...
            if project.pg_status_sync_last_payload_hash == payload_hash:
                run.write(
                    {
                        'status': 'skipped',
                        'message': _('Project status snapshot unchanged; publication skipped.'),
                        'finished_at': fields.Datetime.now(),
                    }
                )
                self._update_project_status(
                    project,
                    {
                        'pg_status_sync_last_status': 'skipped',
                        'pg_status_sync_needs_publish': False,
                        'pg_status_sync_last_message': _('Project status snapshot unchanged; nothing to publish.'),
                        'pg_status_sync_quality_review_feedback': review.get('feedback') or False,
                    },
                )
                return run

            self._append_run_log(run, _("Publishing status snapshot to GitHub repository."))
            publish_result = self.publisher.publish_project_status_snapshot(
            ...
            run.write(
                {
                    'status': 'done',
                    'message': _('Project status snapshot published successfully.'),
            ...
            self._update_project_status(
                project,
                {
                    ...
                    'pg_status_sync_last_payload_hash': payload_hash,
                    'pg_status_sync_last_published_at': fields.Datetime.now(),
                    'pg_status_sync_needs_publish': False,
                    'pg_status_sync_last_message': _('Project status snapshot published to repository.'),
                    'pg_status_sync_quality_review_feedback': review.get('feedback') or False,
                },
            )
            return run
        except Exception as exc:
            _logger.exception("Project status sync failed for run %s", run.id)
            run.write(
                {
                    'status': 'error',
                    'message': str(exc),
            ...
            self._update_project_status(
                project,
                {
                    ...
                    'pg_status_sync_last_message': str(exc),
                },
            )
            raise
